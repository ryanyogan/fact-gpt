...
class ChatGPTAPI {
    ...
    private let basePrompt = "You are ChatGPT, a large language model trained by OpenAI.  You answer as consisely as possible for each response (e.g. Don't be vervose).  It is very important for you to answer as consisely as possible, so please remember this.  If you are generating a list, do not have too many items.\n\n\n"
    ...
    func sendMessage(_ text: String) async throws -> String {
        ...
        do {
            let completionResponses = try self.jsonDecoder.decode(CompletionResponse.self, from: result)
            let responseText = completionResponses.choices.first?.text ?? ""
